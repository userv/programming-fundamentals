...
class PriceChangeAlert
{
    static void Main()
    {
        ...
        private static string GetMessage(double currentPrice, double price, double difference, bool isSignificantDifference)
        {
            ...
            {
                message = string.Format("MINOR CHANGE: {0} to {1} ({0:0.0%})", price, currentPrice, difference);
            }
            else if (isSignificantDifference && (difference > 0))
            {
                message = string.Format("PRICE UP: {0} to {1} ({0:0.0%})", price, currentPrice, difference);
            }
            else if (isSignificantDifference && (difference < 0))
            { 
                message = string.Format("PRICE DOWN: {0} to {1} ({0:0.0%})", price, currentPrice, difference);
            }
            return message;
        }
